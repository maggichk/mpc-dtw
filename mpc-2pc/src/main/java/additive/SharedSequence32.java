...
public class SharedSequence32 {
	...
	public SharedSequence32() {
		
	}

	public SharedSequence32(int queryLength, int index, int clusterIndex, int shareNo, long[] sharedValue, long[] sharedSquareValue) {
		this.index = index;
	...
	public SharedSequence32(int queryLength, int index, int clusterIndex, int shareNo, boolean isCenter, long[] sharedValue, long[] sharedSquareValue) {
		this.index = index;
		this.clusterIndex = clusterIndex;
		this.length = queryLength;
		this.setShareNo(shareNo);
	...
	public SharedSequence32(int queryLength, int index, int clusterIndex,int shareNo) {
	...
	public SharedSequence32(int queryLength, int index, int clusterIndex, int shareNo, boolean isCenter, SecureRandom random) {
		...
		for (int i = 0; i < queryLength; i++) {
			...
			long shareData = AdditiveUtil32.modAdditive(random.nextLong());
			long shareSquareData = AdditiveUtil32.modAdditive(random.nextLong());
	...
	public void minus(SharedSequence32 B) {
		...
		if (B.getLength() != this.length) {
			...
			for (int i = 0; i < this.length; i++) {
				...
				// shareValue
				this.sharedSequence[i] = AdditiveUtil32.sub(this.sharedSequence[i], valuesB[0]);

				// shareSquareValue
				this.sharedSquareSequence[i] = AdditiveUtil32.sub(this.sharedSquareSequence[i], valuesB[1]);
			}
		}
	}

	public void add(SharedSequence32 B) {
		if (B.getLength() != this.length) {
			throw new IllegalArgumentException(
					"Specified arrays are nto equal length. Sequence " + this.index + ", Sequence " + B.getIndex());
		} else {
			...
			for (int i = 0; i < this.length; i++) {
				long[] valuesB = B.getSharedData(i);
				// shareValue
				this.sharedSequence[i] = AdditiveUtil32.modAdditive(this.sharedSequence[i] + valuesB[0]);

				// shareSquareValue
				this.sharedSquareSequence[i] = AdditiveUtil32.modAdditive(this.sharedSquareSequence[i] + valuesB[1]);
			}
		}
	}

	public SharedSequence32 add(SharedSequence32 A, SharedSequence32 B) {
		 SharedSequence32 res = new SharedSequence32(A.getLength(), A.getIndex(), A.getClusterIndex(), 2, A.getSharedSequence(),
