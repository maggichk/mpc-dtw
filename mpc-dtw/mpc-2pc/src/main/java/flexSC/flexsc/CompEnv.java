...
import flexSC.rand.ISAACProvider;
...
public abstract class CompEnv<T> {
	...
	public static CompEnv getEnv(Mode mode, Party p, Network w) {
		if (mode == Mode.REAL)
			if (p == Party.Bob)
				return new flexSC.gc.regular.GCEva(w);
			else
				return new flexSC.gc.regular.GCGen(w);		
		else if (mode == Mode.OPT)
			if (p == Party.Bob)
				return new flexSC.gc.halfANDs.GCEva(w);
			else
				return new flexSC.gc.halfANDs.GCGen(w);
			...
			if (p == Party.Bob)
				return new flexSC.gc.offline.GCEva(w);
			else
				return new flexSC.gc.offline.GCGen(w);
