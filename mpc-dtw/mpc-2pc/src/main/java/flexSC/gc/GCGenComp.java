...
import flexSC.ot.FakeOTSender;
import flexSC.ot.OTExtSender;
import flexSC.ot.OTPreprocessSender;
...
public abstract class GCGenComp extends GCCompEnv {
	...
	public static GCSignal[] genPairForLabel(Mode mode) {
		...
		if(mode == Mode.OFFLINE) {
			if(Flag.offline) {
				label[0] = new GCSignal(flexSC.gc.offline.GCGen.fread.read(10));
			}
			else 
				label[0].send(flexSC.gc.offline.GCGen.fout);
		}
