...
package flexSC.circuits.arithmetic;

import java.util.Arrays;

import flexSC.util.Utils;
import flexSC.circuits.CircuitLib;
...
public class IntegerLib<T> extends CircuitLib<T> implements ArithmeticLib<T> {
	...
	public T[] publicValue(long v, int width) {
		
		return toSignals(v, width);
	}
...
//		return karatsubaMultiply(x,y);
	...
	public T[] modUnsigned(T[] x, T[] y) {
		T Xneg = x[x.length - 1];
		T[] absoluteX = absolute(x);
		T[] absoluteY = absolute(y);
		T[] PA = divInternal(absoluteX, absoluteY);
		T[] res = Arrays.copyOfRange(PA, y.length, PA.length);
		return mux(res, sub(y, absoluteX), Xneg);
	}
