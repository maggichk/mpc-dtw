...
public class eightBitSharedSequence {
	...
	public eightBitSharedSequence() {

	}

	public eightBitSharedSequence(int queryLength, int index, int clusterIndex, int shareNo, short[] sharedValue, short[] sharedSquareValue) {
		this.index = index;
	...
	public eightBitSharedSequence(int queryLength, int index, int clusterIndex, int shareNo, boolean isCenter, short[] sharedValue, short[] sharedSquareValue) {
		this.index = index;
		this.clusterIndex = clusterIndex;
		this.length = queryLength;
		this.setShareNo(shareNo);
	...
	public eightBitSharedSequence(int queryLength, int index, int clusterIndex, int shareNo) {
	...
	public eightBitSharedSequence(int queryLength, int index, int clusterIndex, int shareNo, boolean isCenter, SecureRandom random) {
		...
		for (int i = 0; i < queryLength; i++) {
			short shareData = (short) random.nextInt(127);//[0 , 2^31-1)
			short shareSquareData = (short) random.nextInt(127);// 0 - 2^31-1
...
//			//System.out.println("shareData:"+shareData+" shareSquareData:"+shareSquareData);
	...
	public void minus(eightBitSharedSequence B) {
		...
		if (B.getLength() != this.length) {
			...
			for (int i = 0; i < this.length; i++) {
				...
				// shareValue
				this.sharedSequence[i] = eightBitAdditiveUtil.sub(this.sharedSequence[i], valuesB[0]);

				// shareSquareValue
				this.sharedSquareSequence[i] = eightBitAdditiveUtil.sub(this.sharedSquareSequence[i], valuesB[1]);
			}
		}
	}

	public void add(eightBitSharedSequence B) {
		if (B.getLength() != this.length) {
			throw new IllegalArgumentException(
					"Specified arrays are nto equal length. Sequence " + this.index + ", Sequence " + B.getIndex());
		} else {
			...
			for (int i = 0; i < this.length; i++) {
				short[] valuesB = B.getSharedData(i);
				// shareValue
				this.sharedSequence[i] = eightBitAdditiveUtil.modAdditive(this.sharedSequence[i] + valuesB[0]);

				// shareSquareValue
				this.sharedSquareSequence[i] = eightBitAdditiveUtil.modAdditive(this.sharedSquareSequence[i] + valuesB[1]);
			}
		}
	}

	public eightBitSharedSequence add(eightBitSharedSequence A, eightBitSharedSequence B) {
		 eightBitSharedSequence res = new eightBitSharedSequence(A.getLength(), A.getIndex(), A.getClusterIndex(), 2, A.getSharedSequence(),
